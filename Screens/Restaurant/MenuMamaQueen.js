...
import DATA from "../../config/Restaurant/DATA1";
// import {router} from 'expo-router'
const { width } = Dimensions.get("window");

const ITEM_WIDTH = width / 2 - SPACING * 3;

const MenuMamaQueen = ({navigation}) => {
          ...
          >
            {DATA[activeCategory].recipes.map((item) => (
              <TouchableOpacity
                ...
                key={item.id} onPress={() => navigation.navigate('Detail',{
                  recipe: DATA[0].recipes[item.id - 1]
                })}
              >
                {/* <Link href={{pathname:"/RecipeDetailScreen",params:{recipe:DATA[0].recipes[1]}}}> */}
                  <Image
                    style={{
                      width: "100%",
